...
class DDSRCNN(object):
    ...
    def model(self):
        ...
        if self.mode == "inference":
            ...
            ground_truth = tf.image.resize_images(images, (112, 112), method=1,
                                                  align_corners=True)
        else:
        ...
        conv_1 = Conv2D(images, 3, 1, 3, 64, tf.nn.relu, "conv_1")
        conv_2 = Conv2D(conv_1, 3, 1, 64, 64, tf.nn.relu, "conv_2")
        conv_3 = Conv2D(conv_2, 3, 1, 64, 64, tf.nn.relu, "conv_3")
        conv_4 = Conv2D(conv_3, 3, 1, 64, 64, tf.nn.relu, "conv_4")
        conv_5 = Conv2D(conv_4, 3, 1, 64, 64, tf.nn.relu, "conv_5")
        conv_6 = Conv2D(conv_5, 3, 1, 64, 64, tf.nn.relu, "conv_6")
        conv_7 = Conv2D(conv_6, 3, 1, 64, 64, tf.nn.relu, "conv_7")
        conv_8 = Conv2D(conv_7, 3, 1, 64, 64, tf.nn.relu, "conv_8")
        conv_9 = Conv2D(conv_8, 3, 1, 64, 64, tf.nn.relu, "conv_9")
        conv_10 = Conv2D(conv_9, 3, 1, 64, 64, tf.nn.relu, "conv_10")
        deconv_1 = Conv2DTranspose(conv_10, 3, 1, 64, 64, tf.nn.relu, "deconv_1")
        deconv_2 = Conv2DTranspose(deconv_1, 3, 1, 64, 64, tf.nn.relu, "deconv_2") + conv_9
        deconv_3 = Conv2DTranspose(deconv_2, 3, 1, 64, 64, tf.nn.relu, "deconv_3")
        deconv_4 = Conv2DTranspose(deconv_3, 3, 1, 64, 64, tf.nn.relu, "deconv_4") + conv_7
        deconv_5 = Conv2DTranspose(deconv_4, 3, 1, 64, 64, tf.nn.relu, "deconv_5")
        deconv_6 = Conv2DTranspose(deconv_5, 3, 1, 64, 64, tf.nn.relu, "deconv_6") + conv_5
        deconv_7 = Conv2DTranspose(deconv_6, 3, 1, 64, 64, tf.nn.relu, "deconv_7")
        deconv_8 = Conv2DTranspose(deconv_7, 3, 1, 64, 64, tf.nn.relu, "deconv_8") + conv_3
        deconv_9 = Conv2DTranspose(deconv_8, 3, 1, 64, 64, tf.nn.relu, "deconv_9")
        deconv_10 = Conv2DTranspose(deconv_9, 3, 1, 64, 3 * 4 * 4, tf.identity, "deconv_10")
        output = tf.nn.tanh(tf.depth_to_space(deconv_10, 4))
        output = (output + 1.) * 0.5
    ...
    def train(self):
        ...
        if self.mode == "inference":
            ...
            imsave(self.output_path + "_SR.png", srimg)
            print(srimg.shape)
    ...
    def _summary_writer(self):
        with tf.name_scope("Summaries"):
            ...
            psnr = 20.0 * tf.log(1. / tf.sqrt(self.loss)) / np.log(10)
            tf.summary.scalar("PSNR", psnr)
            tf.summary.image('Reconstruction', self.output, 4)
            tf.summary.image('Groundtruth', self.groundtruth, 4)
